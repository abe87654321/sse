...
router.post(
  '/validate',
  asyncWrap(async (req, res) => {
    const { amount, categoryId, categoryName } = req.body;

    if (!amount || amount <= 0) {
      throw new AppError(400, 'INVALID_PARAMS', '请提供有效的 amount');
    }
    ...
    if (!matchedRule) {
      ...
      for (const step of (matchedRule.approval_chain || [])) {
        if (step.role) {
          ...
          if (parseInt(approvers[0].cnt, 10) === 0) {
            issues.push(`审批步骤 ${step.step}（${step.label || step.role}）没有在职审批人`);
            suggestions.push(`请为角色"${step.role}"添加在职用户`);
          }
        }
      }
    }
    if (!categoryMatch && (categoryId || categoryName)) {
      issues.push('指定费用类别不存在');
      suggestions.push('请使用系统已知费用类别');
    }
    ...
    res.json({
      ...
      matched_rule: matchedRule
        ? { name: matchedRule.name, amount_range: `${matchedRule.min_amount} - ${matchedRule.max_amount}`, approval_steps: matchedRule.approval_chain?.length || 0 }
        : null,
      issues,
      suggestions,
    });
...
router.post(
  '/from-invoice',
  asyncWrap(async (req, res) => {
    const { imageBase64, fileFormat } = req.body;
    if (!imageBase64) throw new AppError(400, 'INVALID_PARAMS', '请提供 imageBase64');

    const { parseOcr } = await import('@sse/ocr');
    const { getEngine } = await import('./ontology-helpers');

    const ocrText = await parseOcr(imageBase64, fileFormat || 'image');
    if (!ocrText) throw new AppError(422, 'OCR_FAILED', '发票OCR识别无返回结果');

    const engine = getEngine();
    const extraction = await engine.extractor.extractFromInvoice(ocrText);
    if (!extraction.amount) {
      throw new AppError(422, 'EXTRACTION_EMPTY', '未能从发票提取到金额');
    }
    ...
    if (extraction.person?.name) {
      ...
      if (users.length > 0) {
        ...
        extraction.person.matchedUserId = resolvedUserId;
      }
    }

    const { valid, error, dto } = await engine.reasoner.reason(extraction);
    if (!valid) {
      res.json({ success: false, error, notified: ['role:admin'] });
      return;
    }

    const apiBase = process.env.PUBLIC_API_BASE || `http://localhost:${process.env.PORT || 3000}`;
    const token = req.headers.authorization?.split(' ')[1] || '';
    const executor = engine.executor;
    const actionResult = await executor.execute(dto!, resolvedUserId, token);
